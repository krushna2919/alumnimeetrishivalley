...
async function listAllMatches(params: {
  ...
  const isMatch = (fullPath: string) => {
    ...
    return (
      n.startsWith(`${id}-`) ||
      n.includes(`/${id}-`) ||
      n.startsWith(`combined-${id}-`) ||
      n.includes(`/combined-${id}-`) ||
      n.startsWith(`edit-mode-${id}-`) ||
      n.includes(`/edit-mode-${id}-`)
    );
  };
  ...
  const add = (fullPath: string, created_at?: string | null) => {
    ...
    out.push({ path: fullPath, url: data.publicUrl, created_at });
  };
...
serve(async (req) => {
  ...
  try {
    ...
    // Scan the payment-proofs bucket with common prefixes
    const commonPrefixes = [
      "",
      "payment-proofs",
      applicationId,
      `payment-proofs/${applicationId}`,
    ];

    const proofs = await listAllMatches({
      supabaseAdmin,
