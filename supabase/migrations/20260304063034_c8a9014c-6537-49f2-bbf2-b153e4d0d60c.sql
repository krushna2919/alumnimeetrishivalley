...
ALTER TABLE public.batch_configuration ADD COLUMN show_stay_option boolean NOT NULL DEFAULT true;

DROP FUNCTION IF EXISTS public.get_open_batch_configuration();

CREATE FUNCTION public.get_open_batch_configuration()
 RETURNS TABLE(registration_start_date timestamp with time zone, registration_end_date timestamp with time zone, year_from integer, year_to integer, is_registration_open boolean, show_stay_option boolean)
 LANGUAGE sql
